...
const KLineChart: React.FC<KLineChartProps> = ({ data, visibleCount }) => {
  ...
  useEffect(() => {
    ...
    const option = {
      ...
      tooltip: {
        trigger: 'axis',
        axisPointer: { type: 'cross', lineStyle: { color: '#475569', width: 1 } },
        confine: true,
        ...
        formatter: (params: any[]) => {
          ...
          return `
            <div style="font-size: 11px; font-family: monospace; min-width: 150px; line-height: 1.5;">
      ...
      grid: [
        { left: '40', right: '15', height: '80%', top: '5%' }
      ],
      xAxis: [
        { 
          type: 'category', 
      ...
      series: [
        ...
        { 
          name: '5日均线', type: 'line', data: displayedData.map(d => d.ma5), 
          smooth: true, showSymbol: false, lineStyle: { width: 1, color: '#eab308' } 
        },
        { 
          name: '10日均线', type: 'line', data: displayedData.map(d => d.ma10), 
          smooth: true, showSymbol: false, lineStyle: { width: 1, color: '#3b82f6' } 
        },
        { 
          name: '20日均线', type: 'line', data: displayedData.map(d => d.ma20), 
  ...
  return <div ref={chartRef} className="w-full h-[220px] lg:h-[320px]" />;
};
